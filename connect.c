...
/*************************************************
    my functions to send info
*************************************************/

void info_received(struct file_info *fi, char *line){
   // printf("%s\n", line);
    const char a[2] = ":";
    const char b[2] = "/";
    char *port = index(line, ':');
    if (port == 0) {
        fprintf(stderr, "do_info_send: format is S<addr>:<port>/TTL/payload\n");
        return;
    }
    *port++ = 0;
    ...
    *--payload = '/';
    ...
    sprintf(tmp, "%s", line);
    char *dst_addr = strtok(tmp, b);
    sprintf(dst, "%s", dst_addr);
    
    sprintf(tmp, "%s", line);
    port = index(tmp, ':');
    *port++ = 0;
    char *ctr = index(port, '/'); 
    char *ttl_count = strtok(ctr, b);
    ...
    *info++ = 0; 
    ...
    struct sockaddr_in addr_dst = string_to_addr(dst);
    char* src_addr = addr_to_string (my_addr);
    int ttl = atoi(ttl_count);

    if(!addr_cmp(my_addr, addr_dst)) {
        printf("%s\n", info);
        return;
    }
     int ttl_new = ttl - 1;
     if (graph_list == NULL || dist == NULL || prev == NULL || graph == NULL) {
         printf("NULL, cannot find the path");
         return;
     }
    
     if (graph_list != NULL){
            nl_destroy(graph_list);
        }
    if (neighbor_addrs != NULL){
       graph_list = create_graph_list(neighbor_addrs, my_addr);
     }
     if (graph_list != NULL) {
            gsize = nl_nsites(graph_list);
            int graph_len = ((gsize) + (gsize) * (gsize));
            graph = malloc(sizeof(int) * graph_len);
            prev = malloc(sizeof(int) * gsize);
            dist = malloc(sizeof(int) * gsize);
            int q;
            for(q = 0; q < (gsize + gsize * gsize); q++){
                   graph[q] = INFINITY;
            }
            for(q = 0; q < gsize; q++){
                   prev[q] = -1;
            ...
            set_graph(neighbor_addrs, my_addr, graph, graph_list, gsize);
            char* src_addr = addr_to_string (my_addr);
            int src = nl_index(graph_list, src_addr);
            dijkstra(graph, gsize, src, dist, prev);
       }
   
     int dst_index = nl_index(graph_list, dst);
     int src_index = nl_index(graph_list, src_addr);
    //printf("dist[dst_index] %d, dst_index %d\n", dist[dst_index], dst_index);
    if (dist[dst_index] == INFINITY || dst_index == -1) {
        printf("INFINITY, cannot find the path\n");
        return;
    }


   if (ttl_new <= 0) {
        printf("The informtion dropped\n");
        return;
    }
    ...
    k = dst_index;
    while (prev[k] != src_index) {
         k = prev[k];
    }
    next_index = k;
  
       char *info_new = malloc(strlen(line) + 3);
       sprintf(info_new, "S%s/%d/%s\n", dst, ttl_new, info);
       char *name = nl_name(graph_list, next_index);
       printf("send to %s\n", name);
       struct file_info *fi_dest = sockaddr_to_file(string_to_addr(name));
       file_info_send(fi_dest, info_new, strlen(info_new));
...
void handle_closed_socket(struct file_info *fi){
       //update neighbors
       char* src = addr_to_string (fi->addr); 
       char *new_neigh = malloc(strlen(neighbor_addrs));
       char *neigh_tmp = malloc(strlen(neighbor_addrs));
       sprintf(neigh_tmp, "%s", neighbor_addrs);
       strcpy(new_neigh, ";");
       char *addrs = index(neigh_tmp, ';'); 
       const char s[2] = ";";
       char *token;
       token = strtok(addrs, s);
       while (token != NULL) {
       if (strcmp(token, src) != 0) {
         strcat(new_neigh, token);
         strcat(new_neigh, ";");
       }
       token = strtok(NULL, s);
       }
       ...
       strcpy(neighbor_addrs, new_neigh);
       free(new_neigh);
       connects = 1;
       delete = 1;
...
static void message_handler(struct file_info *fi, int events){
    ...
    if (events & POLLIN){  
      if (neighbor_addrs == NULL) {
          neighbor_addrs = malloc(40);
      } else {
         neighbor_addrs = realloc(neighbor_addrs, strlen(neighbor_addrs) + 40);
      }
       
        char *neigh = malloc(40);
        strcpy(neigh, ";");
        char* add_string = addr_to_string (fi->addr); 
        strcat(neigh, add_string);
        
        
        if (delete == 0){
           if(!strstr(neighbor_addrs, neigh)) {
             strcat(neighbor_addrs, neigh);
           }
        } else {
             delete = 0;
        }
        ...
        if (connects == 1) {
           counter = counter + 1;
           connects = 0;
           sprintf(buf, "G%s:%d/%d/%s\n", inet_ntoa(my_addr.sin_addr), ntohs(my_addr.sin_port), counter, neighbor_addrs);
           if(new_c == 1) {
              file_info_send(fi, buf, strlen(buf));
              new_c = 0;
            }
           file_broadcast(buf, strlen(buf), fi);
       }
      if (graph_list != NULL){
            nl_destroy(graph_list);
        }
...
int main(int argc, char *argv[]){
     ...
     */
    // YOUR CODE HERE
    if (bind(skt, (struct sockaddr *) &myaddr, sizeof(struct sockaddr_in)) == -1) {
        perror("bind");
    }
    /* Keep track of the socket.
     */
